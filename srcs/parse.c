...
/*   Updated: 2021/03/20 11:33:39 by youncho          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_resolution(t_cub3d *cub, char **info)
{
	...
	cub->screen_width = ft_atoi(info[1]) < MIN_SCREEN_WIDTH ?
						MIN_SCREEN_WIDTH : ft_atoi(info[1]);
	cub->screen_height = ft_atoi(info[2]) < MIN_SCREEN_HEIGHT ?
						MIN_SCREEN_HEIGHT : ft_atoi(info[2]);
}
...
void	store_map(t_cub3d *cub, t_list *cur, int i, int j)
{
	...
	while (++i < cub->map_height && (tmp = (char *)cur->content) && (j = -1))
	{
		_err(!(cub->map[i] = malloc(cub->map_width + 1)), 1);
